...
class App(tk.Tk):
    ...
    def _compute(self, p, full=False):
        ...
        is_rest_noise = False
        if hasattr(self, 'loaded_noise_ch1') and self.loaded_noise_ch1 is not None and len(self.loaded_noise_ch1) > 0:
            ...
            max_samples = int(fs * 30.0)
            if n > max_samples:
                start_idx = n // 2  # Jump to the exact middle of the rest array
                clean = clean[start_idx : start_idx + max_samples]
                n = max_samples
        elif self.loaded_emg is not None:
